...
public class Menu : MonoBehaviour {
	public UnityEngine.Audio.AudioMixerGroup mg;
   ...
   // private bool Extras;
	...
	private Texture ArrowLeft,ArrowRight;
...
//	private float MaxInMenuOptions;
    ...
    void Start () {
		for(int i = 0 ; i<7;i++)
			MenuRects.Add(new Rect (Screen.width / 2 - 100, Screen.height / 4 + 50 * i, 200, 50));
        ...
        //names_options = new string[10] { "0", "DeleteAllSaves", "2", "3","4","5","6","7","8","To menu"};
       // PlayerPrefs.SetInt("NGPlus", 0);
       
	
		
	
        if (PlayerPrefs.GetFloat("Language") == 0) PlayerPrefs.SetFloat("Language", 1);
    ...
    void Update()
    {
		...
		if (PlayerPrefs.GetFloat("Language") == 0&&language!=0) PlayerPrefs.SetFloat("Language", language);
    ...
    void ControllerGeneral()
	{
		...
		if (PlayerPrefs.GetFloat ("Language") == 1 && OptionNamesInLine [4] != "To Main Menu")
			OptionNamesInLine = new string[5] { "Screen Resolution", "Language", "Full Screen", "Back", "To Main Menu" };
		if (PlayerPrefs.GetFloat ("Language") == -1 && OptionNamesInLine [4] != "В главное меню")
			...
			if (Input.GetMouseButtonDown(0) )
            {
				if (ChoisePositionY == 0&&!Visuals&&!Sound&&!Languages)
                {
					if (SceneManager.GetActiveScene ().name != "StartMenu") {
						...
						SceneManager.LoadScene ("MainHouseStart");
						else 
...
//VISUALS
   ...
   void DrawVisuals()
	{
		if(PlayerPrefs.GetFloat("Language")==1&&FullScreenString[1]!="Fullscreen Off")
		FullScreenString = new string[2] { "Fullscreen On", "Fullscreen Off"};
		if(PlayerPrefs.GetFloat("Language")!=1&&FullScreenString[1]!="Полный экран Выкл")
		FullScreenString = new string[2] { "Полный экран Вкл", "Полный экран Выкл"};

		if(PlayerPrefs.GetFloat("Language")==1)InnerOptions = new string[3]{"Resolution: ",FullScreenString[ChoisePositionFullScreen],"Accept"};
...
//LANGUAGE
	...
	void DrawLanguage()
	{
		if(PlayerPrefs.GetFloat("Language")==1)InnerOptions = new string[2]{LanguageString[language],"Back"};
...
//SOUND
	...
	void DrawSound()
	{
		...
		if(PlayerPrefs.GetFloat("Language")==1)InnerOptions = new string[4]{"Master"+" "+SoundVolume[0]*10,"BG"+" "+SoundVolume[1]*10,"Objects"+" "+SoundVolume[2]*10,"Accept"};
	...
	void DrawBoxInnerOptions(int MaxArrows,bool DrawArrows)
	{
		float SH = Screen.height / 4;
			...
			GUI.Box (MenuRects[i], InnerOptions [i], skin.customStyles [1]);
		
		if (DrawArrows) {
			if (ChoisePositionY < MaxArrows) {
				GUI.DrawTexture (new Rect (Screen.width / 2 - 150, SH + 50 * ChoisePositionY, 50, 50), ArrowLeft);
				GUI.DrawTexture (new Rect (Screen.width / 2 + 100, SH + 50 * ChoisePositionY, 50, 50), ArrowRight);
			}
		}
    ...
    void Load()
	{
		...
		mg.audioMixer.SetFloat ("Master", PlayerPrefs.GetFloat("Master_V"));
		mg.audioMixer.SetFloat ("BG", PlayerPrefs.GetFloat("BG_V"));
		mg.audioMixer.SetFloat ("Objects", PlayerPrefs.GetFloat("Objects_V"));
	...
	void SetMixer()
	{
		mg.audioMixer.SetFloat ("Master", -80+SoundVolume [0]*10);
		PlayerPrefs.SetFloat("Master_V",-80+SoundVolume [0]*10);
		
		mg.audioMixer.SetFloat ("BG",-80+SoundVolume [1]*10);
		PlayerPrefs.SetFloat("BG_V",-80+SoundVolume [1]*10);

		mg.audioMixer.SetFloat ("Objects", -80+SoundVolume [2]*10);
